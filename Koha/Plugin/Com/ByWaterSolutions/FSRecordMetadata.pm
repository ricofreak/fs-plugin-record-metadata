...
use Koha::Plugin::Com::ByWaterSolutions::FSRecordMetadata::Problems ();
...
sub create_problem {
    my ( $self, $params ) = @_;
    return Koha::Plugin::Com::ByWaterSolutions::FSRecordMetadata::Problems::create_problem(
        $self->_tables, $params );
}

sub update_problem {
    my ( $self, $problem_id, $params ) = @_;
    return Koha::Plugin::Com::ByWaterSolutions::FSRecordMetadata::Problems::update_problem(
...
sub api_routes {
    ...
    my %problem_props;
    for my $col ( keys %$problem_cols ) {
        next if $col eq 'entry_id';
        $problem_props{$col} = { type => [ $problem_cols->{$col}, 'null' ] };
    }
